...
//static const char rcsid[]="$Id:$"; // DK removed
...
static int decode_type1045(rtcm_t *rtcm)
{
    ...
    double toc,sqrtA;
    char *msg;
    int i=24+12,prn,sat,week,e5a_hs,e5a_dvs,sys=SYS_GAL; //rsv, //DK, not used
    ...
    if (i+484<=rtcm->len*8) {
        ...
        //rsv       =getbitu(rtcm->buff,i, 7);  // DK removed, not used
    }
    else {
                           ...
                           msm_h_t *h, int *hsize)
{
    ...
    double tow,tod;
    char *msg;
    int i=24,j,mask,staid,type,ncell=0; //, dow; // DK, not used
    ...
    *h=h0;
    if (i+157<=rtcm->len*8) {
        ...
        if (sys==SYS_GLO) {
            //dow   =getbitu(rtcm->buff,i, 3);       i+= 3; // not used
