...
extern int readinifile(sdrini_t *ini)
{
    ...
    else if (strcmp(str,"HYDRASDR")==0)     ini->fend=FEND_HYDRASDR;
    else if (strcmp(str,"HACKRF")==0)     ini->fend=FEND_HACKRF;
    else if (strcmp(str,"FILEBLADERF")==0)  ini->fend=FEND_FBLADERF;
    else if (strcmp(str,"FILERTLSDR")==0)   ini->fend=FEND_FRTLSDR;
    else if (strcmp(str,"FILEHYDRASDR")==0) ini->fend=FEND_FHYDRASDR;
    else if (strcmp(str,"FILEHACKRF")==0) ini->fend=FEND_FHACKRF;
    else if (strcmp(str,"FILE")==0)         ini->fend=FEND_FILE;
    else { SDRPRINTF("error: wrong frontend type: %s\n",str); return -1; }

    if (ini->fend==FEND_FILE      ||
        ini->fend==FEND_FBLADERF  ||
        ini->fend==FEND_FHYDRASDR ||
        ini->fend==FEND_FHACKRF   ||
